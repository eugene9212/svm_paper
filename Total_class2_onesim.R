...
for(n.fn in 1:4){
  ...
  for(j in 1:2){
    ...
    for (iter in 1:n.sim) {
      ...
      set.seed(iter)
      if(n.fn == 1) {
        data <- gp.1dim.ss(n, error, beta, cov = cov, rho = rho, t = t, seed = iter)
      } else if (n.fn == 2) {
          data <- gp.1dim.sc(n, error, cov = cov, rho = rho, t = t, seed = iter)
      } else if (n.fn == 3) {
        data <- linear.cross(n, error, cov = cov, rho = rho, t = t, seed = iter)
      } else if (n.fn == 4) {
        data <- linear.par(n, error, beta, cov = cov, rho = rho, t = t, seed = iter)
      } else warning("data generation fn is not matched")
    ...
    a <- round(svm.cre, digits = 3)
    b <- round(fl.cre, digits = 3)
    
    c <- round(svm.acc, digits = 3)
    d <- round(fl.acc, digits = 3)
    
    e <- round(mean(diff.svm), digits = 3)
    f <- round(mean(diff.fl), digits = 3)
    
    g <- round(mean(w.diff.svm), digits = 3)
    h <- round(mean(w.diff.fl), digits = 3)
    
    result[j,] <- cbind(a,b,c,d,e,f,g,h)

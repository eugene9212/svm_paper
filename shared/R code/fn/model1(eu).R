model1 <- function(n, beta, t = seq(0, 1, by = 0.05), n.r = 4, n.c = 4, seed = 1)
{
  ...
  max.dist <- sqrt(2) + 1.0e-8 # why???????????????????
  for (s in 1:n.grid) {
    neighbor.index[[s]] <- which(s.dist[s,] < max.dist)
  }
  
  # covariance matrix of gaussian process # 이게 커널부분인가????????????????
  # 왜 0.1 곱하지? / 다른 커널 사용가능할 듯 / tau값도 바꿔줘도 될듯
  ...
  for (i in 1:n) {
    ...
    for (s in 1:n.grid) {
      ...
      mu.t[s,] <- sin(t)
      z.t[s,] <- rmvnorm(1, mu.t[1,], Sigma) # generate multivariate normal dist.
    }
...
col <- colorlut[ c(10*t(x[[2]])) - zlim[1] + 1 ] 
open3d()
surface3d(1:16,1:21,as.matrix(10*x[[1]]),color=col)

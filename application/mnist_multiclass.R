...
## digit 0, 1, and  2 data are used (three classes)

## for 0,1 digit (train)
trn.ind0 <- which(train_y == 0) # extract index for digit0
trn.ind1 <- which(train_y == 1) # extract index for digit1
trn.ind2 <- which(train_y == 2) # extract index for digit2

train_x0 <- train_x[trn.ind0,]  # create train.x for digit0 
train_y0 <- train_y[trn.ind0]
train_x1 <- train_x[trn.ind1,]  # create train.x for digit1
train_y1 <- train_y[trn.ind1]
train_x2 <- train_x[trn.ind2,]  # create train.x for digit1
train_y2 <- train_y[trn.ind2]

## for 0,1,2 digit (test)
tst.ind0 <- which(test_y == 0) # extract index for digit0
tst.ind1 <- which(test_y == 1) # extract index for digit1
tst.ind2 <- which(test_y == 2) # extract index for digit2

test_x0 <- test_x[tst.ind0,]   # create test.x for digit0
test_y0 <- test_y[tst.ind0]
test_x1 <- test_x[tst.ind1,]   # create test.x for digit1 
test_y1 <- test_y[tst.ind1]
test_x2 <- test_x[tst.ind2,]   # create test.x for digit2 
test_y2 <- test_y[tst.ind2]
